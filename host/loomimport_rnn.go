...
func (s *Server) handleRNNStream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.RNNStreamRequest
	...
	fixturesDir := filepath.Join("python", "rnn", "fixtures")
	fx, err := bridge.LoadRNNFixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamRNNToEntity(req, s.rnnModelsDir, fx)
	if err != nil {
	...
	if _, err := s.rnnStore.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "rnn" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
...
func (s *Server) handleCompareRNN(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, CompareReports(filterBedrock(s.allReports(), "rnn")))
}
