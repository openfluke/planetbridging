...
func (s *Server) handleSwiGLUStream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.SwiGLUStreamRequest
	...
	fixturesDir := filepath.Join("python", "swiglu", "fixtures")
	fx, err := bridge.LoadSwiGLUFixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamSwiGLUToEntity(req, s.swigluModelsDir, fx)
	if err != nil {
	...
	report := Report{
		...
		InputDim:       req.SeqLen * req.InputDim,
		OutputDim:      outDim,
	...
	if _, err := s.swigluStore.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "swiglu" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
...
func (s *Server) handleCompareSwiGLU(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, CompareReports(filterBedrock(s.allReports(), "swiglu")))
}
