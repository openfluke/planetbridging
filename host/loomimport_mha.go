...
func (s *Server) handleMHAStream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.MHAStreamRequest
	...
	fixturesDir := filepath.Join("python", "mha", "fixtures")
	fx, err := bridge.LoadMHAFixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamMHAToEntity(req, s.mhaModelsDir, fx)
	if err != nil {
	...
	report := Report{
		...
		InputDim:       req.SeqLen * req.DModel,
		OutputDim:      outDim,
	...
	if _, err := s.mhaStore.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "mha" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
