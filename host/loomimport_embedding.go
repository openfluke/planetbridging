...
func (s *Server) handleEmbeddingStream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.EmbeddingStreamRequest
	...
	fx, err := bridge.LoadEmbeddingFixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamEmbeddingToEntity(req, s.embeddingModelsDir, fx)
	if err != nil {
	...
	if _, err := s.embeddingStore.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "embedding" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
...
func (s *Server) handleCompareEmbedding(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, CompareReports(filterBedrock(s.allReports(), "embedding")))
}
