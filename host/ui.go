...
<header>
  <div class="tabs">
    ...
    <a href="/?tab=residual" {{if eq .Tab "residual"}}class="active"{{end}}>Residual</a>
  ...
  <div class="header-actions">
    ...
    <span class="export-hint">dense · cnn1 · cnn2 · cnn3 · mha · lstm · rnn · layernorm · embedding · rmsnorm · swiglu · residual · mixer — all models &amp; compare rows</span>
  </div>
  {{if eq .Tab "cnn1"}}
  ...
  {{else if eq .Tab "rmsnorm"}}
  <h1>RMSNorm — planet pipeline compare</h1>
  <div class="meta">
    fixture <strong>{{.RMSNormFixture.Version}}</strong>
    · {{.RMSNorm.ReportCount}} pipeline reports
    · {{.RMSNormLoom.EntityFileCount}} <code>.entity</code>
    · {{.RMSNormLoom.LoomReportCount}} loom reports
    · {{.RMSNormLoom.PendingLoomSteps}} pending stream
  </div>
  <div class="fixture-banner"><strong>RMSNorm bedrock (gamma only).</strong> {{.RMSNormFixture.Note}}</div>
  <div class="pipeline-hint">
    Planets: <strong>pytorch · tensorflow · jax</strong>.
  ...
  {{else if eq .Tab "residual"}}
  <h1>Residual — planet pipeline compare</h1>
  <div class="meta">
    fixture <strong>{{.ResidualFixture.Version}}</strong>
    · {{.Residual.ReportCount}} pipeline reports
    · {{.ResidualLoom.EntityFileCount}} <code>.entity</code>
    · {{.ResidualLoom.LoomReportCount}} loom reports
    · {{.ResidualLoom.PendingLoomSteps}} pending stream
  </div>
  <div class="fixture-banner"><strong>Residual bedrock (main + skip).</strong> {{.ResidualFixture.Note}}</div>
  <div class="pipeline-hint">
    Planets: <strong>pytorch · tensorflow · jax</strong>.
  Stream: <code>POST /api/v1/loom/stream/residual</code> · <a href="/PROGRESS.md">PROGRESS.md</a>
  </div>
...
{{if .ResidualLoomRows}}
<section class="loom-catalog">
  <h2>Loom entity checkpoints ({{.ResidualModelsDir}})</h2>
  <table>
    <thead><tr><th>Model</th><th>Planet</th><th>.entity</th><th>Loom report</th></tr></thead>
    <tbody>
      {{range .ResidualLoomRows}}{{range .Planets}}
      <tr>
        <td>{{.ModelID}}</td><td>{{.Engine}}</td>
        <td class="mono">{{if .EntityFiles}}{{index .EntityFiles 0}}{{else}}—{{end}}</td>
        <td>{{if .HasLoomReport}}<span class="loom-yes">✓</span>{{else}}<span class="loom-no">pending</span>{{end}}</td>
...
{{end}}
{{if not .Residual.Models}}
  <div class="empty">No Residual reports yet.<br><br><code>go run .</code><br><code>./python/residual/run_residual.sh</code></div>
{{else}}
  {{range .Residual.Models}}
  <section class="model">
    <h2>{{.ModelID}}</h2>
    {{range .Pipelines}}
    <div class="planet-block">
...
{{if .RMSNormLoomRows}}
<section class="loom-catalog">
  <h2>Loom entity checkpoints ({{.RMSNormModelsDir}})</h2>
  <table>
    <thead><tr><th>Model</th><th>Planet</th><th>.entity</th><th>Loom report</th></tr></thead>
    <tbody>
      {{range .RMSNormLoomRows}}{{range .Planets}}
      <tr>
        <td>{{.ModelID}}</td><td>{{.Engine}}</td>
        <td class="mono">{{if .EntityFiles}}{{index .EntityFiles 0}}{{else}}—{{end}}</td>
        <td>{{if .HasLoomReport}}<span class="loom-yes">✓</span>{{else}}<span class="loom-no">pending</span>{{end}}</td>
...
{{end}}
{{if not .RMSNorm.Models}}
  <div class="empty">No RMSNorm reports yet.<br><br><code>go run .</code><br><code>./python/rmsnorm/run_rmsnorm.sh</code></div>
{{else}}
  {{range .RMSNorm.Models}}
  <section class="model">
    <h2>{{.ModelID}}</h2>
    {{range .Pipelines}}
    <div class="planet-block">
