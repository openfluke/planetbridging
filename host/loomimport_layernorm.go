...
func (s *Server) handleLayerNormStream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.LayerNormStreamRequest
	...
	fixturesDir := filepath.Join("python", "layernorm", "fixtures")
	fx, err := bridge.LoadLayerNormFixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamLayerNormToEntity(req, s.layernormModelsDir, fx)
	if err != nil {
	...
	if _, err := s.layernormStore.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "layernorm" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
...
func (s *Server) handleCompareLayerNorm(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, CompareReports(filterBedrock(s.allReports(), "layernorm")))
}
