...
func (s *Server) handleCNN3Stream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.CNN3StreamRequest
	...
	fixturesDir := filepath.Join("python", "cnn3", "fixtures")
	fx, err := bridge.LoadCNN3Fixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamCNN3ToEntity(req, s.cnn3ModelsDir, fx)
	if err != nil {
	...
	report := Report{
		...
		InputDim:       req.Depth * req.Height * req.Width * req.InputChannels,
		OutputDim:      outDim,
	...
	if _, err := s.cnn3Store.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "cnn3" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
