...
func (s *Server) handleResidualStream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.ResidualStreamRequest
	...
	fixturesDir := filepath.Join("python", "residual", "fixtures")
	fx, err := bridge.LoadResidualFixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamResidualToEntity(req, s.residualModelsDir, fx)
	if err != nil {
	...
	if _, err := s.residualStore.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "residual" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
...
func (s *Server) handleCompareResidual(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, CompareReports(filterBedrock(s.allReports(), "residual")))
}
