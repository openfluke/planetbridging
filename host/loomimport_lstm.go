...
func (s *Server) handleLSTMStream(w http.ResponseWriter, r *http.Request) {
	...
	var req bridge.LSTMStreamRequest
	...
	fixturesDir := filepath.Join("python", "lstm", "fixtures")
	fx, err := bridge.LoadLSTMFixture(req.FixtureVersion, fixturesDir)
	if err != nil {
	...
	result, err := bridge.StreamLSTMToEntity(req, s.lstmModelsDir, fx)
	if err != nil {
	...
	report := Report{
		...
		InputDim:       req.SeqLen * req.InputSize,
		OutputDim:      outDim,
	...
	if _, err := s.lstmStore.Save(report); err != nil {
		writeJSON(w, http.StatusInternalServerError, loomStreamResponse{
			Status:  "error",
	...
	for _, rep := range s.allReports() {
		rep.Normalize()
		if rep.Bedrock == "lstm" && rep.Planet == req.Planet && rep.ModelID == req.ModelID && rep.Stage == "native" {
...
func (s *Server) handleCompareLSTM(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, CompareReports(filterBedrock(s.allReports(), "lstm")))
}
