"""Common CLI for LayerNorm engine runners."""
...
from .manifest import Manifest, load_manifest, model_input_dim, model_output_dim
from .reporter import host_reachable, post_report
from .spec import DEFAULT_HOST, LAYERNORM_ROOT
from .variants import VariantResult


def build_parser(planet: str) -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=f"planetbridging layernorm runner ({planet})")
    p.add_argument("--host", default=DEFAULT_HOST)
    p.add_argument("--models-dir", type=Path, default=LAYERNORM_ROOT / "models")
    p.add_argument("--manifest", type=Path, default=LAYERNORM_ROOT / "manifest.yaml")
