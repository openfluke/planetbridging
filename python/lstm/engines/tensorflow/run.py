#!/usr/bin/env python3
"""TensorFlow LSTM bedrock (Loom gate layout)."""
...
from shared.lstm_forward import loom_lstm_forward_batch  # noqa: E402
...
def handler(*, model: ModelSpec, manifest: Manifest, data: dict, models_dir: Path, host: str = DEFAULT_HOST, skip_loom: bool = False):
    gates, skipped = train_or_load(model=model, manifest=manifest, data=data)
    out_dir = model_dir(PLANET, model.id)
    out_dim = model_output_dim(model)
    _, _, x_test, _ = slice_model_inputs(data, model, out_dim)
    native = flatten_out(
        loom_lstm_forward_batch(
            x_test,
            i_weights=gates["i"],
            f_weights=gates["f"],
            g_weights=gates["g"],
