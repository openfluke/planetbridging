"""Common CLI for LSTM engine runners."""
...
from .spec import DEFAULT_HOST, LSTM_ROOT
from .variants import VariantResult


def build_parser(planet: str) -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=f"planetbridging lstm runner ({planet})")
    p.add_argument("--host", default=DEFAULT_HOST)
    p.add_argument("--models-dir", type=Path, default=LSTM_ROOT / "models")
    p.add_argument("--manifest", type=Path, default=LSTM_ROOT / "manifest.yaml")
...
def run_planet(
    ...
    for model in manifest.models:
        ...
        try:
            ...
            in_dim = model.seq_len * model.input_size
            for v in variants:
