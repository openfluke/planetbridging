...
def to_ndhwc(x: np.ndarray) -> np.ndarray:
    """Fixture is NCDHW; Flax Conv expects NDHWC."""
...
def train_or_load(*, model: ModelSpec, manifest: Manifest, data: dict) -> tuple[TrainBundle, bool]:
    ...
    module = CNN3Stack(layers_cfg(model), model.input_channels)
    x_train, y_train, _, _ = slice_model_inputs(data, model, out_dim)
    xj = jnp.asarray(to_ndhwc(x_train).astype(np.float32))
...
def handler(*, model: ModelSpec, manifest: Manifest, data: dict, models_dir: Path, host: str = DEFAULT_HOST, skip_loom: bool = False):
    ...
    out = np.asarray(bundle.apply(jnp.asarray(to_ndhwc(x_test).astype(np.float32))), dtype=np.float64)
    out = out[:, :out_dim]
    out_dir = model_dir(PLANET, model.id)
    results = [
        VariantResult(
