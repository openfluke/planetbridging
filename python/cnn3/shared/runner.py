"""Common CLI for CNN3 engine runners."""
...
from .spec import CNN3_ROOT, DEFAULT_HOST
from .variants import VariantResult


def build_parser(planet: str) -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=f"planetbridging cnn3 runner ({planet})")
    p.add_argument("--host", default=DEFAULT_HOST)
    p.add_argument("--models-dir", type=Path, default=CNN3_ROOT / "models")
    p.add_argument("--manifest", type=Path, default=CNN3_ROOT / "manifest.yaml")
...
def run_planet(
    ...
    for model in manifest.models:
        ...
        try:
            ...
            in_dim = model.input_channels * model.depth * model.height * model.width
            for v in variants:
