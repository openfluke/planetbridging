"""Stream live CNN3 layers to Loom .entity."""
...
from .extractors import extract_jax_conv3d, extract_keras_conv3d, extract_pytorch_conv3d
from .layer_stream import post_cnn3_stream
...
def stream_planet_to_loom(
    *,
    ...
    if resp.get("status") != "ok":
        raise RuntimeError(f"cnn3 loom stream: {resp.get('message', resp)}")
    ...
    if max_diff is not None:
        print(
            f"[{planet}] {model.id} cnn3 loom ok entity={entity_path} "
...
def _extract(extractor: str, net: Any, model: ModelSpec, params: Any | None):
    if extractor == "pytorch":
        return extract_pytorch_conv3d(net, model)
    if extractor == "tensorflow":
        return extract_keras_conv3d(net, model)
    if extractor == "jax":
        if params is None:
            raise ValueError("jax extractor requires params")
        return extract_jax_conv3d(params, model)
