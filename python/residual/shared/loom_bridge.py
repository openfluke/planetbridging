"""Stream Residual layers to Loom .entity."""

from __future__ import annotations

import numpy as np

from .layer_stream import ResidualLayerStream, post_residual_stream
...
def stream_planet_to_loom(
    *,
    ...
    if resp.get("status") != "ok":
        raise RuntimeError(f"residual loom stream: {resp.get('message', resp)}")
    ...
    if max_diff is not None:
        print(
            f"[{planet}] {model.id} residual loom ok entity={entity_path} "
