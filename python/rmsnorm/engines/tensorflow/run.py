#!/usr/bin/env python3
"""TensorFlow RMSNorm bedrock (numpy reference forward)."""
...
from shared.rmsnorm_forward import loom_rmsnorm_forward  # noqa: E402
...
def handler(*, model: ModelSpec, manifest: Manifest, data: dict, models_dir: Path, host: str = DEFAULT_HOST, skip_loom: bool = False):
    ...
    native = loom_rmsnorm_forward(x_test, gamma=weights["gamma"])[:, :out_dim]
    results = [
        VariantResult(
            planet=PLANET,
            stage="native",
