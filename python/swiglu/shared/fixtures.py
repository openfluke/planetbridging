"""Deterministic SwiGLU fixtures [N, seq, input_dim]."""
...
def _make_sequences(x_scalar: np.ndarray, seq_len: int, input_dim: int, rng: np.random.Generator) -> np.ndarray:
    n = x_scalar.shape[0]
    out = np.zeros((n, seq_len, input_dim), dtype=np.float64)
    for i in range(n):
        phase = float(x_scalar[i, 0])
        for t in range(seq_len):
            for d in range(input_dim):
                out[i, t, d] = np.sin(0.11 * t + 0.07 * d + phase) + 0.05 * np.cos(0.03 * t + phase * 0.3)
        out[i] += rng.standard_normal((seq_len, input_dim)) * 0.01
    return out
...
def ensure_fixtures(manifest: Manifest | None = None) -> dict[str, np.ndarray]:
    ...
    max_s, max_d = manifest.max_seq_len, manifest.max_input_dim
...
def slice_model_inputs(
    ...
    seq, in_dim = model.seq_len, model.input_dim
    x_train = data["x_train"][:, :seq, :in_dim]
    x_test = data["x_test"][:, :seq, :in_dim]
