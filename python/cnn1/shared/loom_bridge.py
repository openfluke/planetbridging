"""Stream live CNN1 layers to Loom .entity."""
...
from .extractors import extract_jax_conv1d, extract_keras_conv1d, extract_pytorch_conv1d
from .layer_stream import post_cnn1_stream
...
def stream_planet_to_loom(
    *,
    ...
    if resp.get("status") != "ok":
        raise RuntimeError(f"cnn1 loom stream: {resp.get('message', resp)}")
    ...
    if max_diff is not None:
        print(
            f"[{planet}] {model.id} cnn1 loom ok entity={entity_path} "
...
def _extract(extractor: str, net: Any, model: ModelSpec, params: Any | None):
    if extractor == "pytorch":
        return extract_pytorch_conv1d(net, model)
    if extractor == "tensorflow":
        return extract_keras_conv1d(net, model)
    if extractor == "jax":
        if params is None:
            raise ValueError("jax extractor requires params")
        return extract_jax_conv1d(params, model)
