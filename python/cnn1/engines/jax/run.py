...
from shared.manifest import CNN1LayerSpec, Manifest, ModelSpec, model_output_dim  # noqa: E402
...
def to_nlc(x: np.ndarray) -> np.ndarray:
    """Fixture is NCL [batch, channels, length]; Flax Conv expects NLC."""
...
def train_or_load(*, model: ModelSpec, manifest: Manifest, data: dict) -> tuple[TrainBundle, bool]:
    ...
    module = CNN1Stack(layers_cfg(model), model.input_channels)
    x_train, y_train, _, _ = slice_model_inputs(data, model, out_dim)
    xj = jnp.asarray(to_nlc(x_train).astype(np.float32))
...
def handler(*, model: ModelSpec, manifest: Manifest, data: dict, models_dir: Path, host: str = DEFAULT_HOST, skip_loom: bool = False):
    ...
    out = np.asarray(bundle.apply(jnp.asarray(to_nlc(x_test).astype(np.float32))), dtype=np.float64)
    out = out[:, :out_dim]
    out_dir = model_dir(PLANET, model.id)
    results = [
        VariantResult(
