...
def init_or_load_weights(
    ...
    *,
    ...
    weights = init_mixer_weights(model_seed(manifest, model))
...
def layer_streams_from_weights(weights: dict[str, np.ndarray]) -> list[dict[str, Any]]:
    return layers_json_from_weights(weights)
