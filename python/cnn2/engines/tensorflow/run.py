...
def build_model(model: ModelSpec) -> tf.keras.Model:
    inp = tf.keras.Input(shape=(model.height, model.width, model.input_channels), name="input")
    x = inp
...
def to_nhwc(x: np.ndarray) -> np.ndarray:
    """Fixture is NCHW [batch, C, H, W]; Keras channels_last expects NHWC."""
...
def train_or_load(*, model: ModelSpec, manifest: Manifest, data: dict) -> tuple[tf.keras.Model, bool]:
    ...
    x_train = to_nhwc(x_train).astype(np.float32)
...
def infer_saved(saved_path: Path, x: np.ndarray) -> np.ndarray:
    inp = tf.keras.Input(shape=(x.shape[1], x.shape[2], x.shape[3]), name="input")
...
def handler(*, model: ModelSpec, manifest: Manifest, data: dict, models_dir: Path, host: str = DEFAULT_HOST, skip_loom: bool = False):
    ...
    x = to_nhwc(x_test).astype(np.float32)
