"""Load CNN2 manifest and model definitions."""
...
def cnn2_out_spatial(spatial: int, kernel: int, stride: int, padding: int) -> int:
...
def model_output_dim(model: ModelSpec) -> int:
    ...
    for layer in model.layers:
        h = cnn2_out_spatial(h, layer.kernel_size, layer.stride, layer.padding)
        w = cnn2_out_spatial(w, layer.kernel_size, layer.stride, layer.padding)
        ch = layer.filters
    return ch * h * w
...
def load_manifest(path: Path | None = None) -> Manifest:
    ...
    for m in raw.get("models", []):
        layers = tuple(CNN2LayerSpec(**layer) for layer in m["layers"])
        models.append(
            ModelSpec(
                id=m["id"],
    ...
    return Manifest(
        ...
        max_spatial=int(raw.get("max_spatial", 32)),
