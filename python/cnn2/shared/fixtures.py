"""Deterministic 4D CNN2 fixtures [N, C, H, W]."""
...
def _make_grids(x_scalar: np.ndarray, spatial: int, channels: int, rng: np.random.Generator) -> np.ndarray:
    n = x_scalar.shape[0]
    out = np.zeros((n, channels, spatial, spatial), dtype=np.float64)
    for i in range(n):
        phase = float(x_scalar[i, 0])
        for row in range(spatial):
            for col in range(spatial):
                out[i, 0, row, col] = (
                    np.sin(0.12 * row + 0.08 * col + phase)
                    + 0.1 * np.cos(0.05 * row - 0.04 * col + phase * 0.5)
                )
        for c in range(1, channels):
            out[i, c, :, :] = out[i, 0, :, :] * (0.5 + 0.1 * c) + rng.standard_normal((spatial, spatial)) * 0.01
    return out
...
def ensure_fixtures(manifest: Manifest | None = None) -> dict[str, np.ndarray]:
    ...
    x_scalar = rng.standard_normal((n_train + n_test, max_s * max_s), dtype=np.float64)
    x_train = _make_grids(x_scalar[:n_train], max_s, manifest.input_channels, rng)
    x_test = _make_grids(x_scalar[n_train:], max_s, manifest.input_channels, rng)
...
def slice_model_inputs(
    ...
    c, h, w = model.input_channels, model.height, model.width
    x_train = data["x_train"][:, :c, :h, :w]
    x_test = data["x_test"][:, :c, :h, :w]
