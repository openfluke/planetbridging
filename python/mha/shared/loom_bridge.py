"""Stream live MHA layers to Loom .entity."""
...
from .layer_stream import MHALayerStream, post_mha_stream
...
def stream_planet_to_loom(
    *,
    ...
    if resp.get("status") != "ok":
        raise RuntimeError(f"mha loom stream: {resp.get('message', resp)}")
    ...
    if max_diff is not None:
        print(
            f"[{planet}] {model.id} mha loom ok entity={entity_path} "
