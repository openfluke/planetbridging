"""Common CLI for MHA engine runners."""
...
from .spec import DEFAULT_HOST, MHA_ROOT
from .variants import VariantResult


def build_parser(planet: str) -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=f"planetbridging mha runner ({planet})")
    p.add_argument("--host", default=DEFAULT_HOST)
    p.add_argument("--models-dir", type=Path, default=MHA_ROOT / "models")
    p.add_argument("--manifest", type=Path, default=MHA_ROOT / "manifest.yaml")
...
def run_planet(
    ...
    for model in manifest.models:
        ...
        try:
            ...
            in_dim = model.seq_len * model.d_model
            for v in variants:
