"""Deterministic MHA fixtures [N, seq, d_model]."""
...
def _make_sequences(x_scalar: np.ndarray, seq_len: int, d_model: int, rng: np.random.Generator) -> np.ndarray:
    n = x_scalar.shape[0]
    out = np.zeros((n, seq_len, d_model), dtype=np.float64)
    for i in range(n):
        phase = float(x_scalar[i, 0])
        for t in range(seq_len):
            for d in range(d_model):
                out[i, t, d] = np.sin(0.11 * t + 0.07 * d + phase) + 0.05 * np.cos(0.03 * t + phase * 0.3)
        out[i] += rng.standard_normal((seq_len, d_model)) * 0.01
    return out
...
def ensure_fixtures(manifest: Manifest | None = None) -> dict[str, np.ndarray]:
    ...
    max_s, max_d = manifest.max_seq_len, manifest.max_d_model

    x_scalar = rng.standard_normal((n_train + n_test, max_s * max_d), dtype=np.float64)
...
def slice_model_inputs(
    ...
    seq, d = model.seq_len, model.d_model
    x_train = data["x_train"][:, :seq, :d]
    x_test = data["x_test"][:, :seq, :d]
