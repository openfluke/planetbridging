"""Stream live Embedding layers to Loom .entity."""

from __future__ import annotations

import numpy as np

from .layer_stream import EmbeddingLayerStream, post_embedding_stream
...
def stream_planet_to_loom(
    *,
    ...
    if resp.get("status") != "ok":
        raise RuntimeError(f"embedding loom stream: {resp.get('message', resp)}")
    ...
    if max_diff is not None:
        print(
            f"[{planet}] {model.id} embedding loom ok entity={entity_path} "
