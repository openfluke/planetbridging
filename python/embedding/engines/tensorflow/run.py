#!/usr/bin/env python3
"""TensorFlow Embedding bedrock (numpy reference forward)."""
...
from shared.embedding_forward import loom_embedding_forward  # noqa: E402
...
def handler(*, model: ModelSpec, manifest: Manifest, data: dict, models_dir: Path, host: str = DEFAULT_HOST, skip_loom: bool = False):
    ...
    native = loom_embedding_forward(x_test, table=weights["table"])[:, :out_dim]
    results = [
        VariantResult(
            planet=PLANET,
            stage="native",
