...
// InferLSTMStack runs Loom LSTM forward per sample (batch=1).
func InferLSTMStack(net *poly.VolumetricNetwork, xTest [][][]float64, outputDim int) [][]float64 {
	...
	for i, sample := range xTest {
		...
		for li := range net.Layers {
			layer := &net.Layers[li]
			if layer.IsDisabled || layer.Type != poly.LayerLSTM {
				continue
			}
			_, post := poly.LSTMForwardPolymorphic(layer, t)
			t = post
		}
